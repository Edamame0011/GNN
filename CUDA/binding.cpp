...
torch::Tensor fused_csr_cfconv(
    ...
    return FusedCSRCFConvFunction::apply(
        x, filter_out, edge_weight, edge_src, edge_dst, dst_ptr, (float)cutoff
    );
}
...
TORCH_LIBRARY(flash_schnet_ext, m) {
    ...
    m.def("fused_csr_cfconv(Tensor x, Tensor filter_out, Tensor edge_weight, Tensor edge_src, Tensor edge_dst, Tensor dst_ptr, float cutoff) -> Tensor");
}

TORCH_LIBRARY_IMPL(flash_schnet_ext, CUDA, m) {
