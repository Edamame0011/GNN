...
class GNNFF_train(nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, edge_weight: torch.Tensor, calc_energy: bool, batch: Optional[torch.Tensor] = None):
        if calc_energy:
        ...
        # エッジ特徴量の作成
        edge_feat = torch.cat(
            [h[i], h[j], rbf_expansion], dim=1
        )

        force_magnitude = self.output(edge_feat).t()
        edge_forces = force_magnitude * (edge_weight / distances)

        forces = torch.zeros((3, x.size(0)), device=x.device, dtype=edge_weight.dtype)
        forces.index_add_(1, i, edge_forces)
        forces.index_add_(1, j, -edge_forces)
        ...
        forces_e = x.new_zeros(3, x.size(0), dtype=edge_weight.dtype)
        forces_e.index_add_(1, i, diff_E)
        forces_e.index_add_(1, j, -diff_E)
        ...
        return forces, total_energy, forces_e